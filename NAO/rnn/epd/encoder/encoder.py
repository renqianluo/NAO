...
class Encoder(object):
  ...
  def build_encoder(self, x, batch_size, is_training):
    ...
    if self.source_length != self.encoder_length:
      ...
      assert self.source_length % self.encoder_length == 0, "source_length should be divided by encoder_length!"
      ratio = self.source_length // self.encoder_length
      x = tf.reshape(x, [batch_size, self.encoder_length, ratio*self.emb_size])
    if self.time_major:
